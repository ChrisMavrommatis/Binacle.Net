...
{
	[Theory]
	[MemberData(nameof(InteropProvider.ArtifactFiles), MemberType = typeof(InteropProvider))]
	public void Artifact_File_Covers_Exactly_The_Input_Scenarios(string fileName)
	{
		// ShouldBe with ignoreOrder is a set compare — it fails if either side has a Name the other lacks.
		InteropProvider.ArtifactNames(fileName).ShouldBe(InteropProvider.InputNames, ignoreOrder: true);
	}
}
