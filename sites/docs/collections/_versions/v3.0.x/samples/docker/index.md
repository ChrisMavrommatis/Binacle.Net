...
|---|---|---|
| [Minimal]({% vlink /samples/docker/minimal/index.md %}) | You want the smallest configuration that still answers requests | none |
...
What decides this is who calls the API, not how big your deployment is.

**Prod** is the API sitting behind your own backend. Your server calls it, so it needs no accounts, no tokens
and no database. Most deployments are this.
...
Each sample has a matching smoke profile of the same name, run against the image on every release.
