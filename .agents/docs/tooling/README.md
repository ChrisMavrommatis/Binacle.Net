...
| `links.just` | **Not a script** — the `links` module for the root `justfile`. `just links all` checks the internal links in both built sites with `lychee --offline`, `just links docs`/`web` one of them, `just links external <site>` every link including other people's servers. Reads `artifacts/<site>`, so the site has to be built first — it stops with a pointer to `just build <site>` rather than checking nothing. `tooling/lychee.toml` holds the URLs it must never check and is named with `--config`, because lychee only finds a config in the working directory by itself |
