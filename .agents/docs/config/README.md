...
| `build.just` | **Not a script** — the `build` module for the root `justfile`. `just build publish` publishes the API (`-c Release -o build/binacle-net --self-contained --runtime linux-x64`); `just build image [version]` publishes then `docker build -t binacle-net:<version>` (default `local`). Neither starts compose, and neither needs `sudo`, so CI can call them as they stand |
