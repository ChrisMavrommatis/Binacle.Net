...
**This release changes the packing-log configuration shape and log location. A deployment that enables packing
logs with the old nested config will fail startup validation until the config is flattened. See the Migration
Guide.** (Remove this banner if no breaking/migration items remain at release time.)
...
1. **Flatten `PackingLogs.json`.** Move the keys out of the old nested `Packing` block up under `PackingLogs`, and
   ...
   ```
2. **Log location moved** from `data/pack-logs/packing/` to `data/pack-logs/`. Repoint log collection at the new
