...
public class PackByCustomBehavior : BehaviourTestsBase
{
	private readonly CustomPackRequest sampleRequest = new()
	{
		Parameters = new()
		{
	...
	public async Task Post_WithEarlyFailOptInAndLargeVolume_ReturnsWithEarlyFail_ContainerVolumeExceeded()
	{
		...
		await base.PackRequest_ValidateBasedOnParameters(
			...
			result =>
			{
				...
				{
					binPackResult.Result.ShouldBe(BinPackResultStatus.EarlyFail_ContainerVolumeExceeded);
				}
			}
		);
	}
	...
	public async Task Post_WithEarlyFailOptInAndLargeDimension_ReturnsWithEarlyFail_ContainerDimensionExceeded()
	{
		...
		await base.PackRequest_ValidateBasedOnParameters(
			...
			result =>
			{
				...
				foreach (var binPackResult in result.Data)
				{
					binPackResult.Result.ShouldBe(BinPackResultStatus.EarlyFail_ContainerDimensionExceeded);
	...
	private CustomPackRequest CreateSpecialRequest(
		Action<PackRequestParameters>? modifyParameters = null
	)
	{
