...
One wrangler config per site - `docs.wrangler.jsonc` and `demo.wrangler.jsonc`. They are the whole deployment
configuration for [`sites/docs`](../../sites/docs) and [`sites/demo`](../../sites/demo).

**Nothing here is run by hand.** The `Deploy Docs Site` and `Deploy Demo Site` workflows call
...
| Key | Why |
|---|---|
| `assets.directory` | The built site to upload - `artifacts/docs` or `artifacts/demo`. Relative to **this file**, and it has to match the `destination` in that site's `_config.yml` |
