...
// One box type: its dimensions and how many of them. Implements the read-only geometry interfaces so it goes
// straight to CompactNotationFormatter with no adapter.
...
// Parses the raw OR-Library container-loading text for thpack1..7 (the Bischoff & Ratcliff instances). Read
// line by line, the way the file is laid out:
//
...
// Each dimension is followed by a 0/1 flag: 1 = that side may face up, 0 = it may not. The packer does not use
// the flags, so they are read past.
//
// There is no per-problem results line - the raw text carries no packing outcome.
internal static class OrLibraryParser
{
	public static IReadOnlyList<RawProblem> Parse(string text)
	{
		...
		{
			// Line 1: problem index, then the seed. The index becomes the name; the seed is unused.
			...
			// type, length, flag, width, flag, height, flag, quantity - hence the gaps in the indices below,
			// where the type number and the orientation flags are skipped.
	...
	// The next non-blank line, split on whitespace. Skipping blanks keeps stray newlines between problems from
	// throwing the reader off.
