...
public class FitCustomCompareBehavior : BehaviourTestsBase
{
	private const string routePath = "/api/v4/fit/compare-bins";

	private readonly FitCustomCompareRequest sampleRequest = new()
	{
	...
	public FitCustomCompareBehavior(BinacleApi sut) : base(sut)
	{
	}
	...
	public Task Post_ReturnsOneResultPerBin()
		=> base.FitCompareRequest_Validate(routePath, this.sampleRequest,
	...
	public Task Post_ReturnsResultsInRequestOrder()
		=> base.FitCompareRequest_Validate(routePath, this.sampleRequest, result =>
	...
	{
		var request = new FitCustomCompareRequest
		{
			Parameters = new() { Algorithm = Algorithm.FFD },
			Bins = this.sampleRequest.Bins,
			Items = this.sampleRequest.Items
		};
		await base.FitCompareRequest_Validate(routePath, request, result =>
		{
	...
	[Fact(DisplayName = $"POST {routePath}. When All Items Fit Every Bin, Returns Fits For Every Bin")]
	public Task Post_WhenAllItemsFit_Returns_Fits()
		=> base.FitCompareRequest_Validate(routePath, this.sampleRequest, result =>
		{
			foreach (var binResult in result.Results)
			{
				binResult.Status.ShouldBe(BinFitResultStatus.Fits);
				binResult.EarlyExitReason.ShouldBe(BinFitEarlyExitReason.None);
			}
		});
	...
	public async Task Post_WithOneBinTooSmall_ReturnsPerBinResults()
	{
		...
		await base.FitCompareRequest_Validate(routePath, request, result =>
		{
			...
			result.Results.First(x => x.Bin.ID == "too_small").Status
				.ShouldBeOneOf(BinFitResultStatus.DoesNotFit, BinFitResultStatus.EarlyExit);
			result.Results.First(x => x.Bin.ID == "big_enough").Status.ShouldBe(BinFitResultStatus.Fits);
		});
	}
