...
internal class SqliteSubscriptionRepository : ISubscriptionRepository
{
	...
	{
		const string countSql = "SELECT COUNT(*) FROM Subscriptions WHERE IsDeleted = 0";
		...
		const string sql = "SELECT * FROM Subscriptions WHERE IsDeleted = 0 ORDER BY Id LIMIT @Take OFFSET @Skip";
