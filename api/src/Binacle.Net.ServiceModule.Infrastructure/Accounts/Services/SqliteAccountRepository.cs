...
internal class SqliteAccountRepository : IAccountRepository
{
	...
	{
		const string countSql = "SELECT COUNT(*) FROM Accounts WHERE IsDeleted = 0";
		...
		const string sql = "SELECT * FROM Accounts WHERE IsDeleted = 0 ORDER BY Id LIMIT @Take OFFSET @Skip";
